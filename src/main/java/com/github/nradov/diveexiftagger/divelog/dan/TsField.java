...
class TsField extends Field<Ts> implements Ts {
    ...
    private static final Pattern TS_PATTERN = Pattern.compile(
            "\\A\\d{4}([01]\\d([0-3]\\d([0-2]\\d([0-5]\\d([0-5]\\d)?)?)?)?)?(\\.\\d{0,3})?([\\-+]\\d{4})?\\z");
    ...
    class Repetition
        ...
        Repetition(final String value) {
            if (value.isEmpty()) {
                ...
                final int month = extractTsPiece(value, TsLength.YEAR,
                        TsLength.MONTH);
                final int dayOfMonth = extractTsPiece(value, TsLength.MONTH,
                        TsLength.DAY_OF_MONTH);
                final int hour = extractTsPiece(value, TsLength.DAY_OF_MONTH,
                        TsLength.HOUR);
                final int minute = extractTsPiece(value, TsLength.HOUR,
                        TsLength.MINUTE);
                final int second = extractTsPiece(value, TsLength.MINUTE,
                        TsLength.SECOND);
                ...
                if (dotIndex == -1) {
                    ...
                    if (minusIndex == -1 && plusIndex == -1) {
                        ...
                        fraction = value.substring(dotIndex,
                                Math.max(minusIndex, plusIndex));
                    }
                    nanoOfSecond = Integer.valueOf(
                            (fraction + "000").substring(0, 3)) * 1_000_1000;
                }
                final ZoneId zone;
                if (minusIndex == -1 && plusIndex == -1) {
                    ...
                    final int hours = sign * Integer.parseInt(value
                            .substring(signIndex + 1, signIndex + 3));
                    final int minutes = sign * Integer
                            .parseInt(value.substring(signIndex + 3));
                    zone = ZoneOffset.ofHoursMinutes(
                            hours,
                            minutes);
                }
                zonedDateTime = ZonedDateTime.of(year, month, dayOfMonth, hour,
    ...
    private static int extractTsPiece(final String value, final int beginIndex,
            final int endIndex) {
        return value.length() >= endIndex
                ? Integer.parseInt(value.substring(beginIndex, endIndex)) : 0;
    }
